...
<body>
	...
	<div id="upload">
		<h2 class="module-head">编辑歌曲</h2>
		<form action="__URL__/musicUpdate" method="post" id="upload-info" enctype="multipart/form-data">
			<span>
				<label>歌　　名:</label>
				<input type="text" value="<?php echo ($music["musicName"]); ?>" name="musicname"/>
				　　歌曲存储到服务器上目的文件夹下的名称
			</span>
			<span>
				<label>分类目录:</label>
				<select name="musicclass">
					<?php if(is_array($classList)): $i = 0; $__LIST__ = $classList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(($music["musicClass"]) != $vo["classID"]): ?><option value="<?php echo ($vo["classID"]); ?>"><?php echo ($vo["className"]); ?></option>
						<?php else: ?>
							<option value="<?php echo ($vo["classID"]); ?>" selected><?php echo ($vo["className"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</span>
			<span>
				<label>歌　　手:</label>
				<input type="text" value="<?php echo ($music["musicSinger"]); ?>" name="musicsinger"/>
				　　歌手名或者为乐团、组合名称
			</span>
			<span>
				<label>专　　辑:</label>
				<input type="text" value="<?php echo ($music["musicAlbum"]); ?>" name="musicalbum"/>
				　　单曲则填写为歌名
			</span>
			<span>
				<label>发布时间:</label>
				<input type="text" value="<?php echo ($music["musicPub"]); ?>" name="musicpub"/>
				　　格式为:1975-1-1 
			</span>
			<span>
				<label class="cover-label">封　　面:</label>
				<div id="localImag">
					<img src="__PUBLIC__/Covers/<?php echo ($music["musicID"]); ?>.jpg" id="img_prev" alt="封面图片"/>
				</div>
			...
			<span>
				<label>目的地址:</label>
				<input type="text" readonly id="edit-input" value="<?php echo ($music["URL"]); ?>" name="musicurl"/>
				<a href="#" class="edit">编辑</a>
			</span>
			<input type="hidden" value="<?php echo ($music["musicID"]); ?>" name="musicid" />
			<input type="submit" value="更新歌曲" id="bt-submit"/>
		</form>
	</div>
