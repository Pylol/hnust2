...
<head>
	...
	<link rel="stylesheet" type="text/css" href="__PUBLIC__/Css/users.css" />
	<script type="text/javascript" src="__PUBLIC__/Js/chatroom.js"></script>
...
<body>
	...
	<table class="list-table">
		<thead>
			<tr>
				...
				<th class="sex">性别</th>
				<th class="explanation">个人说明</th>
				<th class="option">操作</th>
			</tr>
		</thead>
		<tbody>
			<?php if(is_array($userList)): $k = 0; $__LIST__ = $userList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
					<td class="stuNumber"><a href="#"><?php echo ($vo["userNumber"]); ?></a></td>
					<td class="username"><?php echo ($vo["userName"]); ?></td>
					<td class="email"><?php echo ($vo["userEmail"]); ?></td>
					<td class="sex"><?php echo ($vo["userSex"]); ?></td>
					<td class="explanation"><?php echo ($vo["userExplanation"]); ?></td>
					<td class="option">
						<a href="#">聊天记录</a>
						<a href="#">禁言</a>
						<a href="#">删除</a>
	...
	<div id="display-num"><p><?php echo (($k)?($k):0); ?> 位用户</p></div>
