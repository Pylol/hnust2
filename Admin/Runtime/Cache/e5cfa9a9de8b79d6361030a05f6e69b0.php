...
<head>
	...
	<link rel="stylesheet" type="text/css" href="__PUBLIC__/Css/useronline.css" />
	<script type="text/javascript" src="__PUBLIC__/Js/useronline.js"></script>
...
<body>
	...
	<div id="tablenav">
		<form action="__URL__/userSearch" method="get" id="search">
			<input type="text" placeholder="输入用户名" name="key"/>
	...
	<table class="list-table">
		...
		<tbody>
			<?php if(is_array($onlinelist)): $k = 0; $__LIST__ = $onlinelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
					<td class="userNumber"><a href="__URL__/records/user/<?php echo ($vo["userNumber"]); ?>"><?php echo ($vo["userNumber"]); ?></a></td>
					<td class="username"><?php echo ($vo["userName"]); ?></td>
					<td class="email"><?php echo ($vo["userEmail"]); ?></td>
					<td class="sex"><?php echo ($vo["userSex"]); ?></td>
					<td class="option">
						<a href="__URL__/records/user/<?php echo ($vo["userNumber"]); ?>">聊天记录</a>
						<?php if($vo["ban"] == 0): ?><a href="#" class="wordspeak">禁言</a>
						<?php else: ?>
							<a href="#" class="wordspeak">恢复</a><?php endif; ?>
					</td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
		</tbody>
	</table>
	<div id="display-num"><p><?php echo (($k)?($k):0); ?> 位用户在线</p></div>
