	...
	*/
	...
	function wsProcessClientHandshake($clientID, &$buffer) {
		...
		if ( preg_match('/<policy-file-request.*>/', $buffer) ) {
			...
			$policy = '<?xml version="1.0"?><!DOCTYPE cross-domain-policy SYSTEM "http://www.macromedia.com/xml/dtds/cross-domain-policy.dtd"><cross-domain-policy><allow-access-from domain="*" to-ports="*" /></cross-domain-policy>' . "\0";
			
			$socket = $this->wsClients[$clientID][0];

			$left = strlen($policy);
			do {
				$sent = @socket_send($socket, $policy, $left, 0);
				if ($sent === false) return false;

				$left -= $sent;
				if ($sent > 0) $policy = substr($policy, $sent);
			}
			while ($left > 0);
